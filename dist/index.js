...
export class GitCommitRewriter {
    ...
    assessCommitQuality(message) {
        ...
        const isGeneric = genericMessages.some(generic => message.toLowerCase() === generic ||
    ...
    redactSensitivePatterns(text) {
        ...
        let redacted = text.replace(/^(diff --git a\/.*\.env.*?$[\s\S]*?)(?=^diff --git |$)/gm, '$1[.ENV FILE CONTENT COMPLETELY HIDDEN FOR SECURITY]\n');
        ...
        // Redact Stripe keys
        redacted = redacted.replace(/(['"]?)(sk_live_[a-zA-Z0-9]{24,}|pk_live_[a-zA-Z0-9]{24,}|sk_test_[a-zA-Z0-9]{24,}|pk_test_[a-zA-Z0-9]{24,})(['"]?)/g, '$1[REDACTED_STRIPE_KEY]$3');
        // Redact private keys
        redacted = redacted.replace(/-----BEGIN (RSA |DSA |EC |OPENSSH )?PRIVATE KEY-----[\s\S]*?-----END (RSA |DSA |EC |OPENSSH )?PRIVATE KEY-----/g, '[REDACTED_PRIVATE_KEY]');
        // Redact JWT tokens
        redacted = redacted.replace(/(['"]?)(eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+)(['"]?)/g, '$1[REDACTED_JWT_TOKEN]$3');
        // Redact passwords in common formats
        redacted = redacted.replace(/(password|passwd|pwd|secret|api_key|apikey|auth_token|access_token|private_key)[\s]*[=:][\s]*['"]([^'"]{8,})['"]/gi, '$1=[REDACTED]');
        // Redact database connection strings
        redacted = redacted.replace(/(mongodb(\+srv)?|postgres(ql)?|mysql|redis):\/\/[^@\s]+@[^\s]+/gi, '$1://[REDACTED_CONNECTION_STRING]');
        // Redact Bearer tokens
        redacted = redacted.replace(/Bearer\s+[a-zA-Z0-9_\-\.]+/gi, 'Bearer [REDACTED_TOKEN]');
        return redacted;
    }
    findCommitMessageContext() {
    ...
    async generateCommitMessage(diff, files, oldMessage) {
        try {
...
process.stdin.on('end', () => {
  ...
  if (newMessage) {
    ...
    async generateForStaged() {
        ...
        const message = await this.generateCommitMessage(stagedDiff, stagedFiles, '' // No old message for new commits
        );
        return message;
    }
    async rewrite() {
